// SpecOne Orchestrator — app shell: desktop chrome, sidebar nav, top bar.
(function () {
  ...
  function Wordmark({ size = 18, onDark = false }) {
  ...
  // ---- Desktop window chrome (traffic lights) ----
  function ChromeBar() {
    const project = use((s) => s.project);
  ...
  function Sidebar() {
    const { view, project, notifs, harnesses } = use();
    ...
    return e('div', { ref, style: { position: 'relative' } },
      ...
      open ? e('div', { style: { position: 'absolute', top: 40, right: 0, width: 330, maxHeight: 380, overflowY: 'auto', background: 'var(--popover)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', zIndex: 60 } },
        e('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 14px', borderBottom: '1px solid var(--border)' } },
          ...
          e('button', { onClick: () => eng.markNotifsRead(), style: { fontFamily: 'var(--font-sans)', fontSize: 11.5, color: 'var(--muted-foreground)', background: 'none', border: 'none', cursor: 'pointer' } }, 'Mark all read')),
        notifs.slice(0, 8).map((n) => e('button', { key: n.id, onClick: () => { store.set({ view: n.view || 'notifications' }); setOpen(false); }, style: { display: 'flex', gap: 10, width: '100%', textAlign: 'left', padding: '10px 14px', border: 'none', borderBottom: '1px solid var(--line-soft)', background: n.read ? 'transparent' : 'var(--accent)', cursor: 'pointer' } },
          ...
          e('div', { style: { flex: 1 } },
            ...
            e('div', { style: { fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--neutral-400)', marginTop: 2 } }, window.SO_ago(n.ts))))),
  ...
  function RuntimeToggle() {
    const mode = use((s) => s.runtimeMode);
  ...
  function Shell({ crumbs, actions, children }) {
    const chrome = use((s) => s.chrome);
  ...
  window.SO_Shell = Shell;
  window.SO_Wordmark = Wordmark;
})();
