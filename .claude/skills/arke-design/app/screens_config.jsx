// SpecOne Orchestrator — Harnesses & models, Settings.
(function () {
  ...
  const { Button, Badge, Card, Callout, StatusDot, Switch } = NS;
  ...
  // ================= HARNESSES =================
  function Harnesses() {
    const { harnesses, tiers } = use();
    const [setup, setSetup] = React.useState(false);
    return e(SO_Page, { max: 1020 },
      e(SO_SectionHead, { eyebrow: 'Project', title: 'Harnesses & models',
      ...
      e('div', { style: { display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', background: 'var(--card)', marginBottom: setup ? 12 : 22 } },
        e(StatusDot, { status: 'agree', pulse: true }),
        e('div', { style: { flex: 1, minWidth: 0 } },
          e('div', { style: { fontFamily: 'var(--font-sans)', fontSize: 13, fontWeight: 600 } }, 'Harness reachable on the host'),
          e('div', { style: { fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted-foreground)' } }, 'opencode://localhost:4096 · probed on launch')),
        e(Button, { variant: 'outline', size: 'sm', iconLeft: e(Icon, { name: 'external', size: 14 }), onClick: () => setSetup((o) => !o) }, setup ? 'Hide setup' : 'Confirm or set up')),
      setup ? e('div', { style: { border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: 16, marginBottom: 22, background: 'var(--background)', maxWidth: 560 } },
        e(window.SO_HarnessSetup, { onConnect: () => setSetup(false) })) : null,
  ...
  function Settings() {
    const { theme, density, runtimeMode, accent, chrome, liveStream } = use();
    const [telemetry, setTelemetry] = React.useState(true);
    const set = (patch) => store.set(patch);
    const Group = ({ title, children }) => e('div', { style: { marginBottom: 26 } },
      ...
      e('div', null, children));
    return e(SO_Page, { max: 760 },
      e(SO_SectionHead, { eyebrow: 'Project', title: 'Settings', sub: 'Theme, default runtime mode, connections and telemetry.' }),
      ...
      e(Group, { title: 'About' },
        e('div', { style: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 0' } },
          ...
          e('div', null,
            ...
            e('div', { style: { fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--muted-foreground)' } }, 'v0.10 · open source · shadcn/Radix neutral theme')))),
    );
  }

  Object.assign(window, { SO_Harnesses: Harnesses, SO_Settings: Settings });
})();
