// SpecOne Orchestrator — shared store + live event-simulation engine.
// Plain JS (no JSX). Exposes window.SO_Store, window.SO_use, window.SO_engine.
(function () {
  // ---------- tiny pub/sub store ----------
  function createStore(initial) {
    ...
    return {
      get: () => state,
      set(patch) { const p = (typeof patch === 'function') ? patch(state) : patch; state = { ...state, ...p }; listeners.forEach((l) => l()); },
  ...
  // board cards: col one of authoring|review|implementing|diff|merged
  const CARDS = [
  ...
  const store = createStore({
    ...
    tiers: TIERS,
    permission: null,            // active overlay permission request
    entryFolder: null,           // chosen folder/clone for first-run init flow
    // settings / tweaks
    theme: 'light',
    density: 'comfortable',
  ...
  function tick() {
    ...
    if (s.permission && s.runtimeMode === 'supervised') return; // block while a gate is open

    const running = s.cards.filter((c) => c.status === 'running' && (c.col === 'authoring' || c.col === 'implementing'));
    if (running.length === 0) {
      // occasionally re-energise: pull a review/diff card forward in full-access
      return;
    }
    ...
    // milestone: gate before finishing an implementing task (supervised)
  ...
  window.SO_Store = store;
  window.SO_use = useStore;
  window.SO_engine = { start, stop, tick, raisePermission, resolvePermission, acceptDiff, logEvent, logAudit, notify, markNotifsRead, moveCard, patchCard, applyTheme, setEmpty, COL_ORDER };
})();
