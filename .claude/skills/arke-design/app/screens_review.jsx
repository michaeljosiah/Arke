// SpecOne Orchestrator — Review panel (multi-model) + Generation workspace.
(function () {
  ...
  // findings keyed so we can compute consensus
  const FINDINGS = [
  ...
  function Review() {
    ...
    const [shown, setShown] = React.useState(FINDINGS.length); // how many findings revealed
    const [decisions, setDecisions] = React.useState({}); // key -> accepted|dismissed|revise
  ...
  function Generation() {
    ...
    const execute = () => {
      ...
      const iv = setInterval(() => {
        if (i >= approved.length) {
          ...
          eng.notify('projection', approved.length + ' artifacts written from SPEC-014', 'projections');
          return;
        }
        const a = approved[i];
        setLog((l) => [...l, { m: (a.det ? 'plugin → ' : 'mcp → ') + a.target + (a.det ? '  (deterministic)' : ''), ok: true }]);
        eng.logAudit({ actor: a.det ? 'projection-plugin' : 'priya.n', kind: 'projection', text: 'Wrote ' + a.group.toLowerCase() + ': ' + a.target, detail: a.det ? 'deterministic · trigger: spec-status' : 'agent-side · approved on preview', status: 'ok' });
        i++;
      }, 500);
    };
    ...
    return e('div', { style: { height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0 } },
      e('div', { style: { padding: '18px var(--page-pad) 0' } },
        e(SO_SectionHead, { eyebrow: 'SPEC-014 · Generation', title: 'Propose, decide, execute',
      ...
      e('div', { style: { flex: 1, display: 'flex', gap: 0, minHeight: 0, borderTop: '1px solid var(--border)' } },
        ...
        // right: preview + decision + exec log
  ...
  window.SO_Generation = Generation;
})();
