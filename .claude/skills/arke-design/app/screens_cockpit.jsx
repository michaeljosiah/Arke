// SpecOne Orchestrator — Authoring cockpit: streaming chat + live spec preview.
(function () {
  ...
  const { AgentMessage, Button, Textarea, Badge, StatusDot } = NS;
  ...
  function MiniSelect({ value, options, onChange, icon }) {
  ...
  function SpecPreview({ spec, editing }) {
    ...
    const send = () => {
      ...
      setTimeout(() => {
        ...
        streamReply('Understood. Updating the ' + target + ' section now — ' + (target === 'tasks' ? 're-deriving the implementation plan and acceptance criteria to match.' : target === 'design' ? 'revising the data model and API contracts grounded in the codebase.' : 'tightening the SHALL statements and scenarios.') + ' The change is reflected in the preview.', {
          agent: role, model,
          onDone: () => {
            // apply a small edit to the spec
            setSpec((sp) => {
              const next = { ...sp };
