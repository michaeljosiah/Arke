...
5. **Substrate exclusivity — one substrate per project (decided 2026-06-29).** Omnigent is a **meta-harness at the same layer as Arke's own coordinator/runner substrate**, not a peer of a leaf harness. A project therefore selects **either** leaf harnesses (OpenCode, Claude Code, …) **or** Omnigent — never both; the two are mutually exclusive in one project's registry. The picker presents Omnigent as a *different kind of choice*: pick Omnigent, then pick which leaf harness it drives (its harnesses are sub-options, not list peers). Exclusivity is **per-project**, not org-wide — different projects may use different substrates. (Mirrors AONIK's `Auth.Provider` "one per deployment" posture.)
6. **Agent authority stays Arke on every substrate (decided 2026-06-29).** Selecting Omnigent does **not** disable Arke-side agent creation and does **not** hand the agent model to Omnigent. Arke's roster (harness-agnostic images + logical tiers) remains the authored source on every substrate; on Omnigent the adapter *projects* it down (decision 3) and `listModels()` reads Omnigent's per-harness catalog to resolve tiers (SPEC-005). This preserves the neutral-seam promise — the Arke experience is identical regardless of what runs underneath — and avoids betting our agent UX on an alpha's model. *Optional concession:* Omnigent's pre-existing agents MAY be surfaced read-only (importable/visible), but Arke's roster stays the thing you author. Rejected alternative — "defer to Omnigent's agents, disable creation" — because it makes Omnigent a privileged special citizen and discards the tier abstraction for those projects.
