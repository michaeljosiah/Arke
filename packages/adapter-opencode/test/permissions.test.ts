...
test("no replied event within the timeout yields unconfirmed (not success)", async () => {
  ...
  const ack = await pc.decide({ permissionId: "perm_1", decision: "once" });
...
test("a decision for an id the server no longer lists as pending is stale", async () => {
  ...
  const pc = new PermissionCoordinator(client, 1000);
  const ack = await pc.decide({ permissionId: "stale_perm", decision: "reject" });
...
test("a duplicate decision after confirmation is an idempotent no-op", async () => {
  const client = recorder(["perm_1"]);
  const pc = new PermissionCoordinator(client, 1000);
  const first = pc.decide({ permissionId: "perm_1", decision: "once" });
  pc.onReplied("perm_1");
  await first;
  const second = await pc.decide({ permissionId: "perm_1", decision: "once" });
...
test("concurrent duplicate decisions ride the same in-flight promise", async () => {
  const client = recorder(["perm_1"]);
  const pc = new PermissionCoordinator(client, 1000);
  const a = pc.decide({ permissionId: "perm_1", decision: "once" });
  const b = pc.decide({ permissionId: "perm_1", decision: "once" });
  pc.onReplied("perm_1");
