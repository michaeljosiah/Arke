...
4. **Persists** each to an append-only `Trace` (`.arke/trace.ndjson`) — the audit
   source of truth.
...
```bash
npm run dev --workspace @arke/coordinator   # ws://127.0.0.1:4319, mock adapter
```

Env: `ARKE_COORDINATOR_PORT` (default 4319), `ARKE_TRACE_PATH`
(default `.arke/trace.ndjson`).

The default `MockAdapter` emits a scripted stream so the loop runs without a live OpenCode
host. Swap in `@arke/adapter-opencode` once a harness host is available.
